...
describe("appendReasoningBody", () => {
  ...
  it("handles cumulative snapshots where italic wrappers differ at line boundaries", () => {
    const first = appendReasoningBody({
      body: "",
      previousBodyOnly: "",
      formattedReasoning: "Thinking\n\n_alpha_",
    });
    expect(first.body).toBe("_alpha_");

    // Cumulative snapshot extends the same line — "_alpha beta_" must replace
    // "_alpha_", NOT duplicate as "_alpha__alpha beta_".
    const second = appendReasoningBody({
      body: first.body,
      previousBodyOnly: first.previousBodyOnly,
      formattedReasoning: "Thinking\n\n_alpha beta_",
    });
    expect(second.body).toBe("_alpha beta_");
  });
  ...
  it("replaces reasoning in-place when status lines were appended after it", () => {
    const first = appendReasoningBody({
      body: "",
      previousBodyOnly: "",
      formattedReasoning: "Thinking\n\n_step one_",
    });
    // A status line is appended between reasoning chunks.
    const bodyWithStatus = first.body + "\n[12:00:00] tool: Bash\n";
    ...
    expect(second.body).toBe("_step one__step two_\n[12:00:00] tool: Bash\n");
    expect(second.previousBodyOnly).toBe("_step one__step two_");
  });
});
