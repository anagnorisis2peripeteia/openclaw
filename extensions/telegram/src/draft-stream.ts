...
export function createTelegramDraftStream(params: {
  ...
  const sendOrEditStreamMessage = async (text: string): Promise<boolean> => {
    ...
    const adaptivePauseRemaining = _adaptiveThrottleState.pausedUntil - Date.now();
    if (adaptivePauseRemaining > 0) {
      if (!backoffRetryTimer) {
        backoffRetryTimer = setTimeout(() => {
          backoffRetryTimer = undefined;
          deferredFlush?.();
        }, Math.min(adaptivePauseRemaining, 5_000));
      }
      return false;
    }
    const adaptiveMs = getAdaptiveThrottleMs();
    const effectiveSendInterval =
      adaptiveMs > _adaptiveThrottleState.minMs
        ? Math.max(chatSendIntervalMs, adaptiveMs)
        : chatSendIntervalMs;
    if (!acquireChatSendGate(chatId, streamId, effectiveSendInterval, streamState.final)) {
      if (!gateRetryTimer) {
        gateRetryTimer = setTimeout(() => {
          gateRetryTimer = undefined;
          deferredFlush?.();
        }, Math.max(effectiveSendInterval, 1));
      }
      return false;
    }
