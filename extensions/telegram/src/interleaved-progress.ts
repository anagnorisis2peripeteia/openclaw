 ...
 *   far, so the new chunk prefix-extends the previous one — replace the
 *   previous formatted block, and skip identical re-deliveries.
...
export function appendReasoningBody(params: {
  ...
  const isCumulative =
    params.previousBodyOnly !== "" &&
    stripLineItalics(bodyOnly).startsWith(stripLineItalics(params.previousBodyOnly));
  if (isCumulative) {
    if (bodyOnly === params.previousBodyOnly) {
      return { body: params.body, previousBodyOnly: bodyOnly };
    }
    const lastIdx = params.body.lastIndexOf(params.previousBodyOnly);
    if (lastIdx !== -1) {
      const replaced =
        params.body.slice(0, lastIdx) +
        bodyOnly +
        params.body.slice(lastIdx + params.previousBodyOnly.length);
      return { body: replaced, previousBodyOnly: bodyOnly };
    }
  }
  return { body: params.body + bodyOnly, previousBodyOnly: bodyOnly };
}
